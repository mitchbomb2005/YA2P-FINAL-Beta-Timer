export class BounceHook {
    game

    enabled = false; visibility = false
    x1; x2
    y1; y2
    length; targetLength = 0; maxLength = 400
    trajectory = new Object(); speed = 50; threshold = 5; motion
    slope
    hitNum
    fixed = false
    ...
    update() {
        ...
        if (this.length > this.targetLength && this.enabled) {
            this.game.player.velX = this.game.player.velX - (((this.x1 - (this.x2 + this.game.camera.x)) * 1) )
            this.game.player.velY = this.game.player.velY - (((this.y1 - (this.y2 + this.game.camera.y)) * 1) )
            this.enabled = false; this.visibility = false
        }
